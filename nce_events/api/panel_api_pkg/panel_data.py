...
from nce_events.api.permissions import _doctype_from_wp_tables_row, has_explicit_doctype_read
...
def _require_root_doctype_read(root_doctype: str) -> None:
	...
	if not dt or dt == "WP Tables":
		return
	if not has_explicit_doctype_read(dt):
...
def _filter_wp_tables_catalog_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
	...
	for row in rows:
		...
		if not dt:
			continue
		if has_explicit_doctype_read(dt):
			out.append(row)
	return out
