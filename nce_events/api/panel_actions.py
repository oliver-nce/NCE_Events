...
from nce_events.api.spa_page import _find_spa_definition_name, switch_page_target_slug
...
def get_panel_actions(scope: str | None = None) -> list[dict[str, Any]]:
	...
	for r in rows:
		...
		if cstr(r.get("action_type")) == "Client Script":
			target = switch_page_target_slug(r.get("client_handler"))
			if target:
				spa_name = _find_spa_definition_name(target)
				if spa_name:
					spa_rows = frappe.get_all(
						"SPA Page Definition",
						filters={"name": spa_name},
						pluck="is_active",
						limit_page_length=1,
						ignore_permissions=True,
					)
					if spa_rows and not cint(spa_rows[0]):
						continue
		out.append(r)
